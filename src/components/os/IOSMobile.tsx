...
export const IOSMobile = () => {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-black text-white">
      ...
      {/* Status Bar */}
      <div className="absolute top-0 left-0 right-0 z-50 flex h-12 items-center justify-between px-6">
        <div className="w-16 text-[14px] font-semibold tracking-wider">
          {time ? formatTime(time) : '--:--'}
        </div>

        {/* Dynamic Island */}
        <div className="h-7 w-32 rounded-full bg-black shadow-lg shadow-black/50 ring-1 ring-white/10" />
      ...
      <AnimatePresence>
        {activeApp && (
          ...
          >
            {/* Safe area padding top for Dynamic Island and bottom for indicator */}
            <div className="h-full w-full overflow-hidden pt-12 pb-8">
              <AppRegistry appId={activeApp} />
            </div>

            {/* Home Indicator (Touch to close) */}
            <div
